...
const Form: NextPage<FromProps> = (Props) => {
  ...
  const { register, control, handleSubmit,reset,watch, formState: { errors } } = useForm<IProdectInput>({
    defaultValues:{
      brand:Props.data?Props.data?.brand:"",
      comments:Props.data?Props.data?.comments:[],
      description:Props.data?Props.data?.description:"",
      details:Props.data?Props.data?.details:[],
      image:Props.data?Props.data?.image:[],
      inStock:Props.data?Props.data?.inStock:null,
      name:Props.data?Props.data?.name:"",
      other:Props.data?Props.data?.other:[],
      price:{
              cost:Props.data?Props.data?.price?.cost:null,
              discount:Props.data?Props.data?.price?.discount:null,
              price:Props.data?Props.data?.price?.price:null,
            },
      rating:{
              average:Props.data?Props.data?.rating.average:0,
              five:Props.data?Props.data?.rating.five:0,
              for:Props.data?Props.data?.rating.for:0,
              three:Props.data?Props.data?.rating.three:0,
              tow:Props.data?Props.data?.rating.tow:0,
              one:Props.data?Props.data?.rating.one:0,
              zero:Props.data?Props.data?.rating.zero:0
            },
      relatedProductsIds:Props.data?Props.data?.relatedProductsIds:[],
      selected:Props.data?Props.data?.selected:[],
      specification:Props.data?Props.data?.specification:[]
    }
  });
...
const handleCloseAddDialog=()=>{
 ...
 const [minimize,setMinimize] = useState([false,false,false,false,false])
  ...
  const EditData =async () =>{
   const response = await updateProdect(Props.data._id,watchAllFields)
    setopenDialog(false)
    enqueueSnackbar(response.message,{variant:response.success?"success":"error"});
  }
  ...
  <>
        <form className={Style.Main} dir='rtl' onSubmit={onSubmit}>
          <Typography className="mb-5" color="success" variant='h4'>تعديل على المنتج</Typography>
              ...
              <Button  color={Props?.data?"warning":"success"} type='submit' className='m-3' variant="contained"
              >{Props?.data?"Edit":"Add"}</Button>
     ...
     <Dialog
      ...
      >
        <DialogTitle  dir="rtl">
          {Props.data?"هل انت متاكد من التعديل على هذا المنتج ؟":"هل انت متاكد من نشر هذا المنتج ؟"}
        </DialogTitle>
        <DialogActions>
          <Button variant='outlined' color="error" onClick={handleCloseAddDialog}>
            لا
          </Button>
          <Button autoFocus color="success" variant='outlined' onClick={Props.data?EditData:AddData}>
            نعم
          </Button>
        </DialogActions>
      </Dialog>
