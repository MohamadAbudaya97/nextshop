         ...
         <Typography className="mb-5" color="success" variant='h4'>أضافة المنتجات</Typography>
            <div className="white">
                ...
                <div className="row d-flex justify-content-around"> 
                    ...
                    detailsFields.map((item,index)=>
                      <>
                        <div className='row' key={index+"detailsFields"}>
                          ...
                          <TextField {...register(`details.${index}`,{required: "هذا الحقل مطلوب."})} required  className="col-10 p-0 mt-3 mb-3" id="details" label="يرجى وضع التفاصيل هنا كنقاط" variant="outlined" helperText={errors.details[index]?.message}  error={errors.details[index]?.message?true:false} color={errors.details[index]?.message?"error":"success"}/>
                    ...
                    <Divider/>
                    {
                      ...
                      <>
                        <div className='row' key={index+"otherFields"}>
                          ...
                          <TextField {...register(`other.${index}.title`,{required: "هذا الحقل مطلوب."})}  className="col-5 p-0 mt-3 mb-3" id="other" label="الاسم" variant="outlined" helperText={errors.other[index]?.title?.message}  error={errors.other[index]?.title?.message?true:false} color={errors.other[index]?.title?.message?"error":"success"}/>
                          <TextField {...register(`other.${index}.description`,{required: "هذا الحقل مطلوب."})}  className="col-5 p-0 mt-3 mb-3" id="other" label="وصف المنتج" variant="outlined" helperText={errors.other[index]?.description?.message}  error={errors.other[index]?.description?.message?true:false} color={errors.other[index]?.description?.message?"error":"success"}/>
                          <Typography className="mt-3 mb-3 col-1" style={{display:"flex",justifyContent:"center",alignItems:"center"}}  variant='h6'>{index+1}</Typography>
                        </div>
                      </>
                    )
                       ...
                       specificationFields.map((item,index)=>
                          <>
                            <div className='row' key={index+"specificationFields"}>
                              ...
                              <TextField {...register(`specification.${index}.prodectInformation`,{required: "هذا الحقل مطلوب."})}  className="col-5 p-0 mt-3 mb-3" id="prodectInformation" label="prodectInformation" variant="outlined" helperText={errors.specification[index]?.prodectInformation?.message}  error={errors.specification[index]?.prodectInformation?.message?true:false} color={errors.specification[index]?.prodectInformation?.message?"error":"success"}/>
                              <TextField {...register(`specification.${index}.details`,{required: "هذا الحقل مطلوب."})}  className="col-5 p-0 mt-3 mb-3" id="details" label="details" variant="outlined"  helperText={errors.specification[index]?.details?.message}  error={errors.specification[index]?.details?.message?true:false} color={errors.specification[index]?.details?.message?"error":"success"}/>
                              <Typography className="mt-3 mb-3 col-1" style={{display:"flex",justifyContent:"center",alignItems:"center"}}  variant='h6'>{index+1}</Typography>
                            </div>
                          </>
                       )
                    ...
                    <Typography className="mt-5" align='center' variant='h6'>صوره المنتج
                      <IconButton  color="success" aria-label="RemoveIcon" onClick={()=>{
                          imageAppend(null);
                      }}>
                          <AddIcon />
                    ...
                    imageFields.map((item,index)=>
                      <>
                        <div className='row' key={index+"imageFields"}>
                          ...
                          <TextField {...register(`image.${index}`,{required: "هذا الحقل مطلوب."})} required  className="col-10 p-0 mt-3 mb-3" id="details" label="يرجى منك وضع رابط الصوره" variant="outlined" helperText={errors.image[index]?.message}  error={errors.image[index]?.message?true:false} color={errors.image[index]?.message?"error":"success"}/>
                          <Typography className="mt-3 mb-3 col-1" style={{display:"flex",justifyContent:"center",alignItems:"center"}}  variant='h6'>{index+1}</Typography>
                        </div>
                      </>
                    )
