...
const Home: NextPage = () => {
  ...
  const chickPrice =()=>{
   ...
   if(price && cost){
    if(discount){
      return (price - (discount/100)*price)-cost
    }else{
      return price-cost
    }
   }else{
   ...
   return  watchAllFields.price?.price
  }
         ...
         <Typography className="mb-5" color="success" variant='h4'>أضافة المنتجات</Typography>
            <div className="white">
                ...
                <div className="row d-flex justify-content-around"> 
                    ...
                    <Divider />
                    <Typography className="mt-2" align='center' color={chickPrice()==0 || chickPrice()==null?"black":chickPrice()<0?"red":"green"} variant='body2'>{chickPrice()?`صافي الربح ${chickPrice()}$ `:"يرجى تعبئه الفورم لمعرفه صافي الربح "}</Typography>
                    <TextField {...register("price.price",{required:true})} type="number" className="col-4 p-0 mt-3 mb-3" id="Price" label="Price" variant="outlined" />
                    <TextField {...register("price.cost",{required:true})} type="number" className="col-4 p-0 mt-3 mb-3" id="cost" label="cost" variant="outlined" />
                    <TextField {...register("price.discount")} type="number" className="col-4 p-0 mt-3 mb-3" id="discount" label="( % ) discount " variant="outlined" />  
