  ...
  return (
    <div className={Style.Main} dir="rtl">
        <div className="d-flex justify-content-between m-3 margin" dir="rtl">
            ...
            {Props.url && <Button className={Style.all}> <span style={{fontSize:18,color:"black"}}>جميع الفئات</span>  <ArrowBackIcon/> </Button>}
        </div>
        <Divider/>
       <div className="row m-4">
            {data.map((item,i)=>
            <div key={i+"section"+item} className="col-lg-3 col-md-6">
                <Item 
                    key={i+"Itemsection"+item}
                     option={true} 
                     discount={10} 
                     price={250}
                     test={i+1}
