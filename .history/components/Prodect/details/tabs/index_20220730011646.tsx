  ...
  return (
    <Box sx={{ width: '100%', typography: 'body1' }}>
      <TabContext value={value}>
        ...
        <TabPanel value="1">
            <ul>{Props.details.map((item,index)=><li key={item + index.toString()} className="m-1 m-md-3">{item}</li>)}</ul>
        </TabPanel>
        <TabPanel value="2">
            <div className="p-3 row">
              ...
              {
                Props.spasification.map((item,index)=>
                <>
                  <strong  className='col-6 p-3'>{item.name}</strong>
                  <div  className='col-6 p-3'>{item.details}</div>
                  <Divider/>                
                </>
                )
              }
